...
(function (sandbox) {
	...
	function MyAnalysis () {
			...
			for (var prefix in roots) {
				...
				Object.getOwnPropertyNames(root).forEach(
					function (propertyName) {
						if((propertyName == "caller" || propertyName == "arguments") && root == Function.prototype){
							return; //"caller" and "arguments" are restricted function properties and cannot be accessed in this context
						}
		...
		var loggedEntriesMap = new Map;//<string, int> //<entry, hotnessFactor> 
		//Is a map because of the faster lookup time
...
window.onkeyup = function(event){
	if(event.keyCode == 80){
		sendLoggedEntries();
		if(!sendEntries) //the data has already been send
			return
